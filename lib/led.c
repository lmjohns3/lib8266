...
void ahoy_led_fade(ahoy_led_handle led, uint32_t hex) {
  xQueueSend((QueueHandle_t) led->targets, (void *) hex, (TickType_t) 0);
}
...
static void fader(void* params) {
  ...
  uint32_t hex;
  while (xQueueReceive(led->targets, (void *) &hex, portMAX_DELAY)) {
...
ahoy_led_handle ahoy_led_init(const uint8_t *channels,
                              ...
                              uint16_t fps,
                              uint16_t queue_size) {
  ahoy_led_handle led = malloc(sizeof(struct ahoy_color_led));
  ...
  led->targets = xQueueCreate(queue_size, sizeof(uint32_t));
  led->levels = xQueueCreate(queue_size, 4 * sizeof(ahoy_fixed_t));
  xTaskCreate(fader, "fader", 8192, (void*) led, 10, NULL);
  return led;
}
